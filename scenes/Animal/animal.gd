...
signal fallen_out_of_bounds(position)
...
func play_stretch_sound() -> void:	
	if (_last_dragged_vector - _dragged_vector).length() > 0:
		if -60 < _last_dragged_vector.x and _last_dragged_vector.x < 0 or 0 < _last_dragged_vector.y and _last_dragged_vector.y < 60:
